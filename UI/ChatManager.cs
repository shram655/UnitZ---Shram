...
public class ChatManager : MonoBehaviour, IOnEventCallback
{
    ...
    [Tooltip("🆕 ВЫКЛ = окошко лога видно ВСЕГДА (рекомендуется)")]
    public bool hideEmptyLog = false;
    ...
    private CanvasGroup inputCanvasGroup; // 🆕 чтобы ввод не мешал кликам, когда закрыт
    ...
    void Start()
    {
        ...
        // 🆕 Ввод ВИДИМ всегда, но не блокирует клики, пока чат закрыт
        if (inputPanel != null)
        {
            inputPanel.SetActive(true);
            SetInputInteractable(false);
        }
        // 🆕 Лог видим всегда (если hideEmptyLog = false)
        if (logPanelImage != null) logPanelImage.enabled = !hideEmptyLog;
    ...
    public void OpenChat()
    {
        ...
        SetInputInteractable(true); // 🆕 ввод кликабелен
    ...
    public void CloseChat()
    {
        ...
        // 🆕 панель остаётся ВИДИМОЙ, но не мешает кликам
        SetInputInteractable(false);
