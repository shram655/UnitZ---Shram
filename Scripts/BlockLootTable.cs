...
public class BlockLootTable : MonoBehaviour
{
    ...
    public void GenerateAndAddLoot(PlayerController pc)
    {
        ...
        Debug.Log($"✅ Выпал: {(chosen.isWeapon ? chosen.weaponName : chosen.isAmmo ? "Патроны" : chosen.itemName)} x{count}");

        if (chosen.isWeapon)
        {
            // 🆕 Создаём WeaponData и добавляем как НОВУЮ запись
            ...
            ShowLootNotification(pc, chosen.weaponName, 1, true, 0, false, chosen.weaponId);
        }
        else if (chosen.isAmmo)
        {
            playerInventory.AddAmmo(count);
            ShowLootNotification(pc, "Патроны", count, false, 0, true, 0);
        }
        else
        {
            ...
            ShowLootNotification(pc, chosen.itemName, count, false, chosen.blockId, false, 0);
        }
    }
    ...
    void ShowLootNotification(PlayerController pc, string itemName, int count, bool isWeapon, int blockId, bool isAmmo = false, int weaponId = 0)
    {
        try
        {
            Sprite icon = null;
