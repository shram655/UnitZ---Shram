...
public class BlockLootTable : MonoBehaviour
{
    ...
    public void GenerateAndAddLoot(PlayerController pc)
    {
        ...
        Debug.Log($"✅ Выпал ОДИН предмет: {(chosen.isWeapon ? chosen.weaponName : chosen.isAmmo ? "Патроны" : chosen.itemName)} x{count}");
        ...
        else if (chosen.isAmmo)
        {
            ...
            ShowLootNotification(pc, "Патроны", count, false, 0, true);
        }
        else
        {
    ...
    void ShowLootNotification(PlayerController pc, string itemName, int count, bool isWeapon, int blockId, bool isAmmo = false)
    {
        try
        {
            Sprite icon = null;
