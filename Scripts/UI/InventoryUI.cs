...
{
    // Единственный активный UI (локального игрока)
    ...
    void Start()
    {
        // 🆕 Если этот UI висит на ЧУЖОМ игроке — полностью отключаем его
        ...
        if (parentPC != null && parentPC.view != null && !parentPC.view.IsMine)
        {
            ...
            enabled = false;
            Debug.Log("🚫 InventoryUI: это UI чужого игрока — отключён");
            return;
        }
    ...
    void Update()
    {
        // 🆕 Чужой UI никогда не обрабатывает ввод
