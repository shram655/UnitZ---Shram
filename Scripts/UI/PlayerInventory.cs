...
public class PlayerInventory : MonoBehaviour
{
    ...
    void Start()
    {
        // 🆕 ИСПРАВЛЕНО: больше НЕ перезаписываем inventoryUI.playerInventory!
        // (раньше чужой игрок мог перехватить общий InventoryUI и сломать твой инвентарь)
        ...
        Debug.Log($"✅ PlayerInventory: UI привязан {(inventoryUI != null ? "OK" : "NULL!")}");
    }
    ...
    InventoryUI FindLocalInventoryUI()
    {
        foreach (var ui in FindObjectsOfType<InventoryUI>())
        {
            ...
            if (pc == null) return ui; // общий сценический UI
    ...
    public void AddToInventory(int blockId)
    {
        if (blockId <= 0)
        {
            Debug.LogWarning($"⚠️ AddToInventory: некорректный blockId={blockId} — пропущено!");
            return;
        }
