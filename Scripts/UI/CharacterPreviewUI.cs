...
public class CharacterPreviewUI : MonoBehaviour
{
    ...
    // ══════════════════════════════════════════════════════
    //  🆕 ИСПРАВЛЕНО: компоненты НЕ уничтожаются, а ОТКЛЮЧАЮТСЯ.
    //  Destroy() вызывал ошибки "Can't remove ... depends on it",
    //  т.к. PhotonView / CharacterController защищены
    //  [RequireComponent] на PlayerController и PlayerMovement.
    //  Для превью нужны только рендереры — остальное глушим.
    // ══════════════════════════════════════════════════════
    void StripClone(GameObject clone)
    {
        // 1) Отключаем ВСЕ скрипты: PlayerController, PlayerMovement,
        //    PhotonView, Gun, UI-скрипты, TMP-тексты и т.д.
        foreach (var mb in clone.GetComponentsInChildren<MonoBehaviour>(true))
        {
            mb.enabled = false;
        }

        // 2) Отключаем коллайдеры (CharacterController наследуется от
        //    Collider, поэтому отключится вместе с ними)
        foreach (var col in clone.GetComponentsInChildren<Collider>(true))
        {
            col.enabled = false;
        }

        // 3) Глушим звук, камеру и AudioListener клона
        foreach (var asrc in clone.GetComponentsInChildren<AudioSource>(true)) asrc.enabled = false;
        ...
        // 4) Отключаем Canvas игрока (его UI не должен попасть в превью)
        foreach (var cv in clone.GetComponentsInChildren<Canvas>(true)) cv.enabled = false;

        // 5) Rigidbody ни от чего не зависит — уничтожаем безопасно
