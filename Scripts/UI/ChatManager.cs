...
{
    private const byte CHAT_EVENT_CODE = 201;
    ...
    void CreateUI()
    {
        ...
        // ═══ ПОЛЕ ВВОДА (на всю ширину, БЕЗ кнопки) ═══
        ...
        fieldRect.offsetMin = new Vector2(10, 4);   // растянуто на всю ширину
        fieldRect.offsetMax = new Vector2(-10, -4); // (кнопки больше нет)

        // 🆕 ОБРЕЗКА ТЕКСТА — буквы НЕ вылезают за рамку!
        ...
        tmpText.enableWordWrapping = false; // одна строка
        // 🆕 ИСПРАВЛЕНО: правильный тип для обрезки текста
        tmpText.overflowMode = TextOverflowModes.Overflow; // было: OverflowModes.Overflow
        ...
        // 🆕 КНОПКА ОТПРАВКИ УДАЛЕНА — отправка только на Enter
    }
    ...
    public void OnEvent(EventData photonEvent)
    {
        if (photonEvent.Code == CHAT_EVENT_CODE)
        {
            ...
            {
                AddMessage((string)data[0], (string)data[1]);
            }
        }
    }
