...
public class PlayerHealth : MonoBehaviourPun
{
    ...
    void Die()
    {
        ...
        // Сообщаем всем: скрыть модель МГНОВЕННО
    ...
    void Respawn()
    {
        ...
        // 🆕 МГНОВЕННО сообщаем всем новую позицию —
        // клон телепортируется сразу, не ждёт пока владелец пошевелится
        if (PhotonNetwork.IsConnected && photonView != null)
        {
            photonView.RPC("RPC_SyncTransform", RpcTarget.Others,
                transform.position, transform.eulerAngles);
        }

        // Сообщаем всем: показать модель снова
        if (PhotonNetwork.IsConnected && photonView != null)
        {
        ...
        Debug.Log("✅ Игрок возродился! HP: " + currentHealth);
    }
