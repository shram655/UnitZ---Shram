...
public class PlayerHealth : MonoBehaviourPun
{
    ...
    bool ContainsUIPanel(GameObject obj)
    {
        ...
        {
            if (child.gameObject == deathPanel || child.gameObject == healthPanel)
                return true;
        }
        return false;
    }

    // ✅ ИСПРАВЛЕНО: скрываем также MeshRenderer и SkinnedMeshRenderer
    void HidePlayer()
    {
        ...
        {
            if (ContainsUIPanel(child.gameObject))
            {
                continue;
            }
        ...
        // ✅ Отключаем все SkinnedMeshRenderer (анимированные модели)
    ...
    // ✅ ИСПРАВЛЕНО: восстанавливаем также MeshRenderer и SkinnedMeshRenderer
    void ShowPlayer()
    {
        ...
        // ✅ Включаем все SkinnedMeshRenderer
        SkinnedMeshRenderer[] skinnedRenderers = GetComponentsInChildren<SkinnedMeshRenderer>();
        foreach (SkinnedMeshRenderer smr in skinnedRenderers)
        {
            if (smr != null)
    ...
    void ClearInventory()
    {
        ...
        {
            player.inventoryUI.UpdateAllSlots();
        }
    ...
    void Die()
    {
        ...
        // 2. Скрываем модель игрока (включая Mesh Renderer)
        HidePlayer();

        // 3. Показываем панель смерти
        ...
        // 4. Блокируем управление и убираем оружие
        Move_Player movePlayer = GetComponent<Move_Player>();
        if (movePlayer != null)
        {
            movePlayer.SetDead(true);
        }
    }
    ...
    void Respawn()
    {
        ...
        if (respawnPoint != null)
        {
            ...
            Debug.Log("  Позиция точки: " + newPosition);
        ...
        // Восстанавливаем голод при респавне
