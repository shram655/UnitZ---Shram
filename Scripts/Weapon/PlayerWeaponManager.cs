...
public class PlayerWeaponManager : MonoBehaviourPun
{
    ...
    public void EquipWeaponFromSlot(int slotIndex)
    {
        ...
        // 🆕 Если в слоте НЕТ оружия (пусто, блок, еда и т.д.)
        if (itemId >= -2)
        {
            // Если было оружие, обязательно убираем его!
            if (currentGun != null)
            {
    ...
    public void UnequipCurrentWeapon()
    {
        ...
        // 🆕 Сообщаем персонажу, что оружия больше нет
        CubeWorldCharacter cwChar = GetComponent<CubeWorldCharacter>();
        if (cwChar != null)
        {
            cwChar.SetHasWeapon(false);
            Debug.Log("✅ UnequipCurrentWeapon: SetHasWeapon(false) вызван");
        }
    ...
    private void DestroyGuns()
    {
        ...
        {
            cwChar.SetHasWeapon(false);
            Debug.Log("✅ DestroyGuns: SetHasWeapon(false) вызван");
        }
    ...
    void CreateAndEquipWeapon(int slotIndex, int weaponId, WeaponData data)
    {
        ...
        {
            Debug.LogWarning("⚠️ CubeWorldCharacter или WeaponAnchor не найден");
            return;
        }
        ...
        // 🆕 Сообщаем персонажу, что оружие взято
        cwChar.SetHasWeapon(true);
        Debug.Log("✅ CreateAndEquipWeapon: SetHasWeapon(true) вызван");
